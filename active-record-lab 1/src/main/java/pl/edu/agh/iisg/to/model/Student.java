...
public class Student {
    ...
    public Map<Course, Float> createReport() {
        String studentGrades = "SELECT * FROM grade JOIN course ON grade.course_id = course.id WHERE student_id = ?";

        Object[] args = {
                this.id
        };

        Map<Course, List<Integer>> tempReport = new HashMap<>();
        ...
        try (ResultSet rs = QueryExecutor.read(studentGrades, args)) {
            while (rs.next()) {
                ...
                if (tempReport.containsKey(course)) {
                    List<Integer> gradesData = tempReport.get(course);
                    gradesData.set(0, gradesData.get(0) + grade);
                    gradesData.set(1, gradesData.get(1) + 1);
                } else {
                    tempReport.put(course, Arrays.asList(grade, 1));
                }
            }

            for (Map.Entry<Course, List<Integer>> entry : tempReport.entrySet()) {
                Course course = entry.getKey();
                List<Integer> gradesData = entry.getValue();
                float averageGrade = (float) gradesData.get(0) / gradesData.get(1);
